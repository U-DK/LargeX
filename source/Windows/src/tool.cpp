...
void WriteFile::SplitLargeFiles()
{
	...
	_stat64(largeFilePath, &statbuff);
	size_t fileSize = statbuff.st_size;
	...
	cout << format("File size: {} bytes",fileSize) << endl;
	cout << format("Every thread will deal with {} bytes", (LINEMAXLEN * LINEPERCPU)) << endl;
	cout << format("Small files amount: {}", fileCounts) << endl;
