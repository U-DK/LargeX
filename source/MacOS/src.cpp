...
void ShowTimeConsuming(clock_t& interval);
...
int main(int argc, char* argv[])
{
	...
	coresCPU -= 1;//Not using them all, save resources for other program.
	if (coresCPU <= 1)
	{
		coresCPU = 1;
	}
	...
	CombineAllResult();
	endTime = clock();
    clock_t interval=endTime-startTime;
    interval/=coresCPU;
    ShowTimeConsuming(interval);
...
void SingleThreadRead(int threadID)
{
	...
	string idmeg = "thread "+to_string(threadID)+" is done";
...
void ShowTimeConsuming(clock_t& startTime, clock_t& endTime)
{
	...
	string timeString = to_string(sec) + 's';
	if (sec>200.0f)
	{
		int s = floor(sec);
		int m = s / 60;
		...
		timeString = to_string(m)+"m"+to_string(s)+"s";
